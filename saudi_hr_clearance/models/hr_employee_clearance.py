...
class HrEmployeeClearance(models.Model):
    ...
    # employee_dept = fields.Boolean('Employee Department', default=False, track_visibility='onchange')
    # it_dept = fields.Boolean('IT Department', default=False, track_visibility='onchange')
    # hr_dept = fields.Boolean('HR Department', default=False, track_visibility='onchange')
    # finance_dept = fields.Boolean('Finance Department', default=False, track_visibility='onchange')
    is_employee_leaving = fields.Boolean('Last Clearance', default=False, track_visibility='onchange')
    eos_count = fields.Integer('EOS Count', compute="_compute_eos_count")
    eos_type = fields.Selection([('resignation', 'Resignation'), ('termination', 'Termination'), ('death', 'Death')], 'EOS Type')
    admin_dept_ids = fields.One2many('clearance.department', 'admin_dept_id', 'Admin Departments')
    finance_dept_ids = fields.One2many('clearance.department', 'finance_dept_id', 'Finance Departments')
    employee_dept_ids = fields.One2many('clearance.department', 'employee_dept_id', 'Employee Department')

    def write(self, vals):
        for rec in self:
            if (vals.get('admin_dept_ids') or vals.get('finance_dept_ids') or vals.get('employee_dept_ids'))\
                and self.env.user.employee_id and not self.user_has_groups('hr.group_hr_user')\
    ...
    def _compute_eos_count(self):
        self.eos_count = self.env['hr.employee.eos'].search_count([('employee_id', '=', self.employee_id.id)])
    ...
    def generate_eos(self):
        ...
        for rec in self:
            if rec.is_employee_leaving:
                eos_ids = self.env['hr.employee.eos'].search([('employee_id', '=', self.employee_id.id)])
                if eos_ids:
                    raise UserError(_("%s's EOS is already Generated.") % self.employee_id.name)
                else:
                    eos = self.env['hr.employee.eos'].create({
                            'name': self.employee_name and self.employee_name + ' ' + 'EOS' or False,
                            'employee_id': self.employee_id and self.employee_id.id or False,
                            'contract_id': self.employee_id and self.employee_id.contract_id and self.employee_id.contract_id.id or False,
    ...
    def clearance_confirm(self):
        ...
        self.write({'state': 'confirm',
                    'line_manager_id': self.employee_id.parent_id.id})
        clearance_dept_ids = self.env['clearance.department.data'].search([('department_type', '=', 'emp_dept')])
