...
class HrExpense(models.Model):
    ...
    company_contribution = fields.Float(string='Company Contribution', digits='Account', track_visibility='onchange', copy=False)
    emp_contribution = fields.Float(string='Employee Contribution', digits='Account', track_visibility='onchange', copy=False)
...
class HrExpenseSheet(models.Model):
    ...
    def _compute_not_inc_sal_amt(self):
        ...
        for expense in self.expense_line_ids.filtered(lambda expense: not expense.include_salary):
            total_amount += expense.currency_id._convert(expense.total_amount, self.currency_id, expense.company_id, expense.date)
...
class HrExpensePayment(models.Model):
    ...
    payment_mode = fields.Selection([("own_account", "Employee (to reimburse)"), ("company_account", "Company"),
                                    ...
                                    'post': [('readonly', True)]}, string="Payment By", track_visibility='onchange')

    calculate_company_expense = fields.Boolean('Calculate Company Expense', copy=False, default=True)
    company_contribution_amount = fields.Float(string='Paid By Company', track_visibility='onchange', copy=False)
    emp_contribution_amount = fields.Float(string='Paid By Employee', track_visibility='onchange', copy=False)
    ...
    def generate_expense_payment(self, catch_obj, description, emp_contribution, company_contribution, payment_mode, name, product_id,
        ...
        if payment_mode in ['own_account', 'both'] and emp_contribution_amount:
            if (emp_contribution_amount <= 0 or emp_contribution_amount > expense_total): # (emp_contribution <= 0 or emp_contribution > expense_total) or
        ...
        if payment_mode in ['company_account', 'both'] and company_contribution_amount:
            if (company_contribution_amount <= 0 or company_contribution_amount > expense_total): #(company_contribution <= 0 or company_contribution > expense_total) or\
        ...
        if emp_contribution_amount or company_contribution_amount:
            expense_data = ({
