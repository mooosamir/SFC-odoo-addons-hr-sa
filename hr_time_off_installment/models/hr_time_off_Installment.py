...
class TimeoffInstallment(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', 'Employee', required=True,domain="[('employee_status','=','active')]",readonly=True,states={'draft': [('readonly', False)]})
    ...
    def _get_time_off_days(self):
        for rec in self:
            time_request=self.env['hr.leave'].search([('state','=','validate'),('employee_id','=',self.employee_id.id),('holiday_status_id','=',self.time_off_type.id),('is_paid','=',False)])
